package com.mtecc.rdc.coach.dao;
...
@Repository
public class CoachDao extends MteccBaseDao {
	...
	public List<User> findUserByLogname(String  logname)throws  Exception{
		...
		sb.append(" from User s where 1=1  and s.logname='"+logname.trim()+"'");
		List<User> list=(List<User>) findByHQL(sb.toString());
		return list;
	}
	...
	public JSONArray  getValidUser(String logName,String password) {
		...
		try {
			...
			userList = (List<User>) findByHQL(sb.toString());
			if(userList.size()==1){
					returnMsg="SUCCESS";
	...
	public List<Coach>  getCoachByUser(Integer userid) throws  Exception{
		StringBuffer sb=new StringBuffer();
		sb.append(" from Coach c where 1=1  and c.userid="+userid+"");
		List<Coach> list=(List<Coach>) findByHQL(sb.toString());
		return list;
	}
